...
?>
	...
	<div class="row" style="margin-top:-75px;">
		<div class="col-lg-6" >
			<div class="well bs-component"style="background:#EFEEE9;color:#006A4E">
				<?php 
					if(f_logged_in()===true) {
								...
								if(isset($_GET['chose_fro'])===true) {
									...
									$fro = $_GET['chose_fro'];
										...
										while($row1 = mysql_fetch_assoc($r1)) {
											echo '<legend style="color:#006A4E">Your flight from '.$row1['from_city'].' to '.$row1['to_city'].'</legend>';
											...
											if($class==='Economy') {
												if(($counta+$countc)<=$row1['e_seats_left']) {
													...
													echo 'Cost of '.$counta.' adult(s): '.$counta.' * TK. '.$row1['e_price'].' =  TK. '.$eat1.'<br>';
													$ect1 = ($countc * $row1['e_price']) * 0.75;
													echo 'Cost of '.$countc.' child: 75% of '.$countc.' * TK. '.$row1['e_price'].' = TK. '.$ect1.'<br>';
													$etax1 = 500;
													echo 'Airport Tax: TK. 100 <br>';
													echo 'Fuel Tax: TK. 100 <br>';
													echo 'Service Tax: TK. 300 <br>';
													echo 'Total Tax: TK. '.$etax1.'<br><br>';
													$etot1 = $eat1 + $ect1 + $etax1;
													echo '<legend style="color:#006A4E">Cost of one trip is TK. '.$etot1.'</legend><hr>';
												}
												else {
												...
												if(($counta+$countc)<=$row1['b_seats_left']) {
													...
													echo 'Cost of '.$counta.' adult(s): '.$counta.' * TK. '.$row1['b_price'].' =  TK. '.$bat1.'<br>';
													$bct1 = ($countc * $row1['b_price']) * 0.75;
													echo 'Cost of '.$countc.' child: 75% of '.$countc.' * TK. '.$row1['b_price'].' = TK. '.$bct1.'<br>';
													$btax1 = 1500;
													echo 'Airport Tax: TK. 200 <br>';
													echo 'Fuel Tax: TK. 200 <br>';
													echo 'Service Tax: TK. 1100 <br>';
													echo 'Total Tax: TK. '.$btax1.'<br><br>';
													$btot1 = $bat1 + $bct1 + $btax1;
													echo '<legend style="color:#006A4E">Cost of one trip is TK. '.$btot1.'</legend><hr>';
												}
												else {
													echo 'Not enough available seats. Sorry, please check some other flight!';
													header("refresh:10; url=index.php");
										...
										while($row2 = mysql_fetch_assoc($r2)) {
											echo '<legend style="color:#006A4E">Your flight from '.$row2['from_city'].' to '.$row2['to_city'].'</legend>';
											...
											if($class==='Economy') {
												if(($counta+$countc)<=$row2['e_seats_left']) {
													...
													echo 'Cost of '.$counta.' adult(s): '.$counta.' * TK. '.$row2['e_price'].' =  TK. '.$eat2.'<br>';
													$ect2 = ($countc * $row2['e_price']) * 0.75;
													echo 'Cost of '.$countc.' child: 75% of '.$countc.' * TK. '.$row2['e_price'].' = TK. '.$ect2.'<br>';
													$etax2 = 500;
													echo 'Airport Tax: TK. 100 <br>';
													echo 'Fuel Tax: TK. 100 <br>';
													echo 'Service Tax: TK. 300 <br>';
													echo 'Total Tax: TK. '.$etax2.'<br><br>';
													$etot2 = $eat2 + $ect2 + $etax2;
													echo '<legend style="color:#006A4E">Cost of return trip is TK. '.$etot2.'</legend><hr>';
												}
												else {
													echo 'Not enough available seats. Sorry, please check some other flight!';
													header("refresh:10; url=index.php");
												...
												if(($counta+$countc)<=$row2['b_seats_left']) {
													...
													echo 'Cost of '.$counta.' adult(s): '.$counta.' * TK. '.$row2['b_price'].' =  TK. '.$bat2.'<br>';
													$bct2 = ($countc * $row2['b_price']) * 0.75;
													echo 'Cost of '.$countc.' child: 75% of '.$countc.' * TK. '.$row2['b_price'].' = TK. '.$bct2.'<br>';
													$btax2 = 1500;
													echo 'Airport Tax: TK. 200 <br>';
													echo 'Fuel Tax: TK. 200 <br>';
													echo 'Service Tax: TK. 1100 <br>';
													echo 'Total Tax: TK. '.$btax2.'<br><br>';
													$btot2 = $bat2 + $bct2 + $btax2;
													echo '<legend style="color:#006A4E">Cost of return trip is TK. '.$btot2.'</legend><hr>';
												}
												else {
													echo 'Not enough available seats. Sorry, please check some other flight!';
													header("refresh:10; url=index.php");
										...
										if($class==='Economy') {
											$etotr = $etot1 + $etot2;
											echo '<legend style="color:#006A4E">Total cost of both the trips is Rs. '.$etotr.'</legend>';
										}
										else if($class==='Business') {
											$btotr = $btot1 + $btot2;
											echo '<legend style="color:#006A4E">Total cost of both the trips is Rs. '.$btotr.'</legend>';
										}
									} 
									...
									while($row1 = mysql_fetch_assoc($r1)) {
										...
										if($class==='Economy') {
											if(($counta+$countc)<=$row1['e_seats_left']) {
												...
												echo 'Total Tax: TK. '.$etax1.'<br><br>';
												$etot1 = $eat1 + $ect1 + $etax1;
												echo '<legend style="color:#006A4E">Total cost of trip is TK. '.$etot1.'</legend>';
											}
											else {
												echo 'Not enough available seats. Sorry, please check some other flight!';
												header("refresh:10; url=index.php");
											...
											if(($counta+$countc)<=$row1['b_seats_left']) {
												...
												echo 'Total Tax: TK. '.$btax1.'<br><br>';
												$btot1 = $bat1 + $bct1 + $btax1;
												echo '<legend style="color:#006A4E">Cost of one trip is TK. '.$btot1.'</legend>';
											}
											else {
												echo 'Not enough available seats. Sorry, please check some other flight!';
												header("refresh:10; url=index.php");
		...
		<div class="col-lg-6" >
			<div class="well bs-component" style="background:#EFEEE9">
				<form class="form-horizontal" action="card.php" method="GET" style="color:#006A4E">
		            ...
		            <input type="hidden" name="count_a" value="<?php echo $counta; ?>"/>
              		...
              		?>
		            <center><button style="background:#006A4E" type="submit" id="book_f" name="book_f" class="btn btn-primary">Book Flight</button></center>
				</form>
			</div>
		</div>
